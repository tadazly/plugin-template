...
- Claude Code：`sync` 把 Codex 配置转换为 `.claude-plugin/plugin.json` 的内联 `mcpServers`，再深度合并根目录 `plugin-kit.json` 的 `claude` 对象。
- WorkBuddy：同一组 server 写进 `.codebuddy-plugin/plugin.json`，路径改用 `${CODEBUDDY_PLUGIN_ROOT}`，再合并 `plugin-kit.json` 的 `codebuddy` 对象。
- 插件根目录禁止 `.mcp.json` 与 `mcp/*.json`：
  - Claude Code 与 WorkBuddy 会自动加载它们，并按用户的会话或工作区目录解析其中的相对路径；
  - WorkBuddy 依次合并清单 `mcpServers`、`.mcp.json`、`mcp/*.json`，同名 server 以后读到的为准，所以这些文件会覆盖清单里的配置。
...
- 工具默认只读。修改用户数据的工具在 Codex 配置中设为 `approval_mode: "prompt"`，并在 Skill 中要求先向用户确认。审批策略只有 Codex 读取，Claude Code 与 WorkBuddy 都不会使用，危险操作不能只靠客户端配置拦截。
- 大结果分页或截断，并告诉调用方如何继续获取。

## 跨平台启动（易踩坑）

同一个 server 要在 Windows 和 macOS 上、在 Codex、Claude Code 和 WorkBuddy 里都能启动。以下都是实际踩过的坑：
...
- **Claude Code 和 WorkBuddy 都忽略 `cwd`**：
  - server 在用户的会话目录或工作区根目录启动，相对路径不会被改写；
  - 插件内的文件一律通过 `${CLAUDE_PLUGIN_ROOT}` 或 `${CODEBUDDY_PLUGIN_ROOT}` 定位。
- **WorkBuddy 的 MCP 规则**（WorkBuddy 5.6.2，来自宿主代码与会话日志）：
  - 清单按 `.codebuddy-plugin` → `.workbuddy-plugin` → `.claude-plugin` 的顺序只读第一个；
  - 下发给 server 的配置只保留 `type`、`command`、`args`、`cwd`、`env` 等字段，`enabled`、审批和超时字段都会被丢弃；
  - 自带的 Node 不在 PATH 中，只通过 `WORKBUDDY_EXTRA_PATHS` 暴露。
- **根目录 `.mcp.json` 在不同客户端里效果不同**：
  - egret-agent-inspector 5.0.1 的根目录 `.mcp.json` 是 Codex 写法（`node ./scripts/start_mcp.js`）。
  - Claude Code 里清单内联的启动器优先，所以能用；WorkBuddy 里 `.mcp.json` 覆盖了清单，又在工作区根目录解析相对路径，连接失败。
- **不要在 Git Bash 里验证 Windows**：Git Bash 的 PATH 里有 `sh`，Claude Code 会按 shebang 执行 sh 启动器，结果看起来正常；在普通 PowerShell 里却会连接失败。
- **启动器要切换工作目录**：Codex 以插件根目录作为 cwd 启动 server；在 Windows 上，进程的 cwd 会锁住这个目录，插件运行期间就无法更新或卸载。`.cmd` 启动器因此先切换到用户目录。

两种方案都只写一份 Codex 配置（command 为 `./bin/<name>`，`cwd` 为 `.`）。`sync` 会把它转换为 Claude Code 的 `${CLAUDE_PLUGIN_ROOT}/bin/<name>` 和 WorkBuddy 的 `${CODEBUDDY_PLUGIN_ROOT}/bin/<name>`，不需要按客户端分别配置：
...
- WorkBuddy 5.6.2（Windows）：
  - CodeBuddy CLI 2.157.0 的 `plugin validate` 通过；
  - 按宿主代码中的加载与启动规则回放，server 能正常启动：先读 `.codebuddy-plugin`，无扩展名命令经 shell 解析到 `.cmd`，cwd 为工作区根目录；
  - 桌面端要新建对话才会连接 MCP，这一步尚未实测。

回放方法：
- WorkBuddy 的 CLI 没有像 `claude mcp list` 那样检查插件 MCP 的命令，所以按上述规则自行拉起 server，发送 `initialize` 与 `tools/list`；
- 桌面端的会话日志在 `~/.workbuddy/logs/<日期>/<会话>.log`，可以搜索 `allServers=` 和 `doConnect FAIL`；
- 运行 CodeBuddy CLI 前先退出 WorkBuddy，并用 `CODEBUDDY_CONFIG_DIR` 指向临时目录。
...
rem Leave the plugin directory so Windows does not lock it while the server runs
cd /d "%USERPROFILE%"
